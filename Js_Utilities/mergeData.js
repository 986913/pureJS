/**
  ...
  It has the interface mergeData(sessions). Sessions from the same user should be merged into one object. When merging:
  ...
  and in the case of merging sessions with the same users, the row should take the place of the earliest occurrence of that user. 
...
function mergeData(sessions) {
  ...
  const map = new Map(); // key is user number, value is somthing like this: { user: 8, duration: 50, equipment: ['bench'] }
  sessions.forEach((session) => {
    ...
    if (!map.has(user)) {
      ...
      const newSession = {
        user,
        duration: duration + map.get(user).duration,
        equipment: Array.from(
          new Set([...equipment, ...map.get(user).equipment].sort())
        ),
      };
      map.set(user, newSession);
    }
  });
