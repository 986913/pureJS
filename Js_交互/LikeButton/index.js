(() => {
  ...
  function likeButton(buttonEl, errorMessageEl) {
    let isliked = false; // Determines if the button is in the default/liked state.
    let isPending = false; // Whether there's a pending background API request.
    let errorMessage = null; // Error message to be shown if the API request failed.
    ...
    function update() {
      ...
      buttonEl.querySelector('.like-button-icon').innerHTML = isPending
        ? spinnerIconHTML
        : heartIconHTML;
      buttonEl.querySelector('.like-button-label').innerHTML = isliked
        ? 'Liked'
        : 'Like';
