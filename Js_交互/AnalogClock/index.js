(() => {
  ...
  const padTwoDigit = (number) =>
    number >= 10 ? String(number) : `0${number}`;

  const hand = ({ height = 1, width = 1, angle }) => {
  ...
  const clock = ($rootEl, size) => {
    const timer = window.setInterval(() => {
      render($rootEl, new Date(), size);
    }, 100);

    // The beforeunload event is fired before the tab/window is closed. Clear the interval when the tab/window is about to be closed.
